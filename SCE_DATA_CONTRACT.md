...
> **Console-only build note** (added by the engineer who implemented this
> contract for `SunandanRoy/CDP-StyleVerse`): no sibling "Consumer
> Storefront" project exists in this workspace. Per an explicit decision
> with the project owner, this Console was built to follow every rule in
> this contract (schema, archetypes, scoring, KPI table, brand rules, bug
> fixes) using a self-authored, internally-consistent dataset, rather than
> importing `shared/` from a real Storefront project. Everywhere this
> document says "both tools" or references Storefront-specific content
> (personas, first-SKU product names, cross-repo checksum match, deep
> links into `PWC_CX_Semi.html`), treat it as aspirational/N/A for this
> build — see `CHANGELOG_CONSOLE_v2.md` for the full list of what was
> adapted and why.
